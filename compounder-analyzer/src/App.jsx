...
async function callAI(prompt){
  ...
  const res=await fetch("/api/analyze",{method:"POST",headers:{"Content-Type":"application/json"},
    body:JSON.stringify({model:"claude-sonnet-4-5-20251001",max_tokens:1400,messages:[{role:"user",content:prompt}]})});
    ...
    <div style={{maxWidth:960,margin:"0 auto",padding:"0 28px 40px"}}>
      <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:16,padding:"28px 32px"}}>
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:T.gold,marginBottom:16,fontWeight:700}}>
          {isEs?"La plataforma de inversiones para Colombia y LATAM":"The investment platform for Colombia and LATAM"}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
          {[
            {icon:"📊",title:isEs?"Análisis de acciones con IA":"AI-powered stock analysis",
             body:isEs?"Analiza acciones de la Bolsa de Valores de Colombia (BVC), NYSE, NASDAQ y mercados de Europa y LATAM. Score de calidad, análisis del moat y consenso de Wall Street en 30 segundos.":"Analyze stocks from the BVC, NYSE, NASDAQ, Europe and LATAM. Quality score, moat analysis, and Wall Street consensus in 30 seconds."},
            {icon:"📁",title:isEs?"Compatible con tus brokers":"Compatible with your brokers",
             body:isEs?"Importa tu portafolio desde Trii, HAPI, XTB e Interactive Brokers (IBKR). Sube una captura o CSV — Inversoria detecta tus posiciones automáticamente.":"Import your portfolio from Trii, HAPI, XTB and IBKR. Upload a screenshot or CSV — Inversoria auto-detects your positions."},
            {icon:"🇨🇴",title:isEs?"Acciones colombianas en vivo":"Colombian stocks live",
             body:isEs?"Precios en vivo de la BVC: Ecopetrol, Bancolombia, Terpel, GEB, ISA, Grupo Sura, CNEC y más de 30 acciones. También México, Brasil, Chile y Argentina.":"Live BVC prices: Ecopetrol, Bancolombia, Terpel, GEB, ISA, Grupo Sura, CNEC and 30+ stocks. Also Mexico, Brazil, Chile and Argentina."},
            {icon:"📈",title:isEs?"Calculadora de interés compuesto":"Compound interest calculator",
             body:isEs?"Calcula cuándo llegarás a tu meta de riqueza en pesos colombianos, dólares o cualquier moneda LATAM. Descubre cuánto ahorrar para llegar a $1.000 millones COP.":"Calculate when you'll reach your wealth goal in Colombian pesos, dollars, or any LATAM currency."},
          ].map(({icon,title,body})=>(
            <div key={title}>
              <div style={{fontSize:13,color:T.text,fontWeight:600,marginBottom:6}}>{icon} {title}</div>
              <div style={{fontSize:12,color:T.muted,lineHeight:1.8}}>{body}</div>
            </div>
          ))}
        </div>
      </div>
...
function ScoreTab({m,setM,moat,setMoat,company,setCompany,sector,setSector,onAnalysis,canAnalyze,onGoToProfile,lang="en"}){
  ...
  const analyze=async(overrideTicker)=>{
    ...
    if(!companyToUse || typeof companyToUse !== 'string' || !companyToUse.trim()){
      setErr(lang==="es"?"Escribe un ticker o nombre":"Enter a company name or ticker.");return;
    }
    if(!canAnalyze())return;
  ...
  return<div className="fi" style={{display:"flex",flexDirection:"column",gap:18}}>
    ...
    {/* ── CHART + CONSENSUS — shows whenever locked (with or without AI) ── */}
    {(locked||fh)&&company&&<div className="chart-consensus-grid" style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:16,marginBottom:16,alignItems:"stretch"}}>
      <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {locked&&info&&<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px",textAlign:"center"}}>
          ...
          <ScoreRing score={calcScore(m,moat)} size={90} lang={lang}/>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:grade(calcScore(m,moat),lang).c,marginTop:4,fontWeight:700}}>{grade(calcScore(m,moat),lang).label}</div>
        </div>}
        {!info&&locked&&<div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:"16px",textAlign:"center",display:"flex",alignItems:"center",justifyContent:"center",flex:1}}>
          <div style={{fontSize:12,color:T.muted,lineHeight:1.7}}>
            ℹ️ {lang==="es"?"Análisis IA no disponible":"AI analysis unavailable"}<br/>
            <span style={{fontSize:10,color:T.muted}}>{lang==="es"?"Recarga créditos para análisis completo":"Add credits for full analysis"}</span>
          </div>
        </div>}
        {fh&&<div style={{background:`linear-gradient(135deg,${T.card},${T.accent})`,border:`2px solid ${fh.rating?.includes("Buy")?T.green:T.gold}44`,borderRadius:12,padding:"14px",textAlign:"center"}}>
          ...
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,color:fh.rating?.includes("Buy")?T.green:T.gold,fontWeight:700,marginBottom:4}}>{fh.rating||"N/A"}</div>
  ...
  return(
    <div style={{padding:"14px 18px"}}>
      <div style={{display:"flex",gap:12,marginBottom:14,padding:"10px 14px",borderRadius:10,
        background:totalRealized>=0?`${T.green}08`:`${T.red}08`,border:`1px solid ${totalRealized>=0?T.green:T.red}33`}}>
        <div>
      ...
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:600}}>
          ...
          <tbody>
            {sells.map(txn=>{
              const avgCost=getAvgCost(txn.ticker); const pnl=(txn.price-avgCost)*txn.shares;
              const pct=avgCost>0?((txn.price-avgCost)/avgCost*100):0; const isPos=pnl>=0; const isEdit=editing===txn.id;
              return(<tr key={txn.id} style={{borderBottom:`1px solid ${T.border}22`}}
                onMouseEnter={e=>e.currentTarget.style.background=T.accent}
                onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
                <td style={{padding:"8px 10px",fontWeight:700,color:T.gold,fontFamily:"'DM Mono',monospace"}}>{txn.ticker}</td>
                <td style={{padding:"8px 10px"}}>{isEdit?<input type="number" value={editShares} onChange={e=>setEditShares(e.target.value)} style={{width:64,padding:"2px 6px",fontSize:11,borderRadius:4,background:T.accent,border:`1px solid ${T.border}`,color:T.text}}/>:txn.shares.toFixed(4)}</td>
                <td style={{padding:"8px 10px",textAlign:"right"}}>{isEdit?<input type="number" value={editPrice} onChange={e=>setEditPrice(e.target.value)} style={{width:72,padding:"2px 6px",fontSize:11,borderRadius:4,background:T.accent,border:`1px solid ${T.border}`,color:T.text,textAlign:"right"}}/>:`$${txn.price.toFixed(2)}`}</td>
                <td style={{padding:"8px 10px",textAlign:"right",color:T.muted}}>${avgCost.toFixed(2)}</td>
                <td style={{padding:"8px 10px",textAlign:"right",color:isPos?T.green:T.red,fontWeight:600}}>{isPos?"+":""}{fmt(Math.abs(pnl))}</td>
                <td style={{padding:"8px 10px",textAlign:"right",color:isPos?T.green:T.red}}>{isPos?"+":""}{pct.toFixed(1)}%</td>
                <td style={{padding:"8px 10px",textAlign:"right",color:T.muted,fontSize:10}}>{txn.date}</td>
                <td style={{padding:"8px 10px",textAlign:"right"}}>
                  {isEdit?<div style={{display:"flex",gap:4,justifyContent:"flex-end"}}>
                    <button onClick={()=>{onEdit(txn.id,{shares:parseFloat(editShares)||txn.shares,price:parseFloat(editPrice)||txn.price});setEditing(null);}} style={{fontSize:10,padding:"2px 8px",borderRadius:4,background:T.green,color:"#fff",border:"none",cursor:"pointer"}}>✓</button>
                    <button onClick={()=>setEditing(null)} style={{fontSize:10,padding:"2px 8px",borderRadius:4,background:T.border,color:T.text,border:"none",cursor:"pointer"}}>✕</button>
                  </div>:<div style={{display:"flex",gap:4,justifyContent:"flex-end"}}>
                    <button onClick={()=>{setEditing(txn.id);setEditShares(String(txn.shares));setEditPrice(String(txn.price));}} style={{fontSize:9,padding:"2px 8px",borderRadius:4,background:`${T.gold}15`,color:T.gold,border:`1px solid ${T.goldDim}33`,cursor:"pointer"}}>✏️</button>
                    <button onClick={()=>{ if(window.confirm(isEs?"¿Eliminar esta venta?":"Delete this sell?")) onDelete(txn.id); }} style={{fontSize:9,padding:"2px 8px",borderRadius:4,background:`${T.red}15`,color:T.red,border:`1px solid ${T.red}33`,cursor:"pointer"}}>🗑</button>
                  </div>}
                </td>
              </tr>);
            })}
...
function StickyAIButton({onAnalyze, loading, hasPositions, lang}){
  ...
  useEffect(()=>{
    // Show button when the portfolio table is in view (user can see positions)
    const banner = document.getElementById("ai-analysis-section");
    if(!banner) return;
    const obs = new IntersectionObserver(
      ([e]) => setVisible(e.isIntersecting),  // show WHILE section is visible
      {threshold:0.1}
    );
    obs.observe(banner);
  ...
  return(
    <div style={{
      ...
      <button
        onClick={()=>{
          onAnalyze();
          // Scroll to AI result after analysis starts
          setTimeout(()=>{
            const el = document.getElementById("ai-result-section") || document.getElementById("ai-analysis-section");
            ...
            :<div style={{display:"flex",flexDirection:"column",gap:6}}>
              {(()=>{
                ...
                return displayData.map(p=>{
                  const val=useUSD?(p.pnlDollar||0):p.pnlPct;
                  const barWidth=Math.abs(val)/maxVal*100;
                  const isPos=val>=0;
                  const color=isPos?T.green+"bb":T.red+"bb";
                  ...
                  return(
                    <div key={p.ticker} style={{display:"grid",gridTemplateColumns:"52px 1fr 70px",alignItems:"center",gap:8}}>
                      ...
                      <span style={{fontSize:11,color,fontWeight:700,textAlign:"right"}}>{label}</span>
                    </div>
                  );
                });
              })()}
              <div style={{display:"grid",gridTemplateColumns:"52px 1fr 70px",gap:8,marginTop:2}}>
                <span/><div style={{display:"flex",justifyContent:"space-between",fontSize:8,color:T.muted}}><span>Menor</span><span>0</span><span>Mayor</span></div><span/>
              </div>
            </div>
          }
        </div>
  ...
  const analyzePortfolio=async()=>{
    ...
    try{
      ...
      const portfolioRes=await fetch("/api/analyze",{
        method:"POST",
        headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-5-20251001",max_tokens:3000,
